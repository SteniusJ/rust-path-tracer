...
impl<'a> BVH<'a> {
    ...
    pub fn recursive_intersect(&self, ray: &Ray, hit_rec: &mut HitRecord, node_idx: usize) {
        ...
        if !intersect_aabb(ray, hit_rec.t, node.aabb_min, node.aabb_max) { return }
        if node.is_leaf() {
            for i in 0..node.tri_count {
                if self.tris[self.tri_indices[node.first_tri_idx + i]].hit(ray, 0.0001, closest_t, &mut temp_rec) {
                    if closest_t > hit_rec.t {
                        closest_t = hit_rec.t;
                        *hit_rec = temp_rec.clone();
                    }
                }
 ...
 */
pub fn build_bvh(tris: Vec<Triangle>) -> (Vec<BVHNode>, Vec<usize>, Vec<Triangle>) {
    let root_node_idx = 0;
